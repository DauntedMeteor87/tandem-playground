/* ============================================================
   ...
   ============================================================ */
const U = {
  homePill:'all', clubTab:'landing', commTab:'yours', commFilter:'clubs',
  msgFilter:'all', mtTab:'upcoming', rideTab:'ride',
  obPhotos:0, packList:{}, slots:{}, exp:{}, ranks:{},
  clubsOpen:false, crewsMore:false, actIndex:0,
};
const SCREENS = {};
...
/* Name + optional profile pic (headshot, circle) */
SCREENS['ob-name'] = () => obShell('ob-name', {
  q:'What should we call you?', hint:"First name only — you can't change it later.", forced:true,
  body:`
    <button class="pfp-drop ${S.ob.pfp?'filled':''}" data-action="addPfp">${S.ob.pfp?avatar('YU',94):I.cam}</button>
    <p class="center small muted" style="margin:0 0 20px">Add a profile picture — optional, a simple headshot.</p>
    <input class="field" placeholder="Your first name" value="${S.ob.name}" oninput="S.ob.name=this.value" />`,
  next:'ob-year'
});
...
/* Experience — sliders built from the activities you picked, prepopulated at 0 */
SCREENS['ob-experience'] = () => {
  const picks = S.ob.activities.length ? S.ob.activities.slice(0,5) : ['Day hikes'];
  const rows = picks.concat(['Looked at a plant']); // one freebie everyone can click
  return obShell('ob-experience', {
    q:'How much have you done?', hint:'Tap to bump each up. Totally fine to leave at zero and skip — most people start there. Trip leads use this to match you to the right trips.', next:'ob-photos', skip:'ob-photos',
    body:`<div class="mt2">${rows.map(n=>{const v=U.exp[n]||0;return `
      <div class="slider-row"><span class="name">${n}</span><div class="slider" data-action="bumpExp" data-param="${n}"><i style="left:calc(${v*20}% - 11px)"></i></div><span class="val">${v===0?'—':v+'×'}</span></div>`}).join('')}
...
/* Photos — capped at 2, so nobody builds a whole portfolio before entering */
SCREENS['ob-photos'] = () => obShell('ob-photos', {
  q:'Add a couple photos', hint:"Just 2 to start — one candid, one that's you. You can build out your profile later.", next:'ob-bio', skip:'ob-bio',
  body:`<div class="tiles" style="padding:0">${[0,1].map(i=>`
    <button class="ph" style="height:150px;border-radius:16px" data-action="addPhoto">${i<U.obPhotos?I.check:I.plus}</button>`).join('')}</div>
    <p class="small muted mt3">${U.obPhotos} / 2 added</p>`
});
/* Bio — say hi, 500 chars, answer one prompt (genuine, not ChatGPT) */
const OB_PROMPTS = ['My ideal Saturday is…','Ask me about…','A trip I want to do…','I geek out over…'];
SCREENS['ob-bio'] = () => obShell('ob-bio', {
  q:'Say hi in one line', hint:'Up to 500 characters — keep it real, like a caption. Answer one prompt if you want.', next:'ob-verify', skip:'ob-verify',
  body:`<textarea class="field" placeholder="e.g. Freshman from Sacramento, looking for hiking buddies!" oninput="S.ob.bio=this.value">${S.ob.bio}</textarea>
    <div class="divlabel">or start from a prompt</div>
    <div style="display:flex;flex-wrap:wrap;gap:8px">${OB_PROMPTS.map(p=>`<button class="chip" data-action="usePrompt" data-param="${p}">${p}</button>`).join('')}</div>`
});
SCREENS['ob-verify'] = () => `
...
SCREENS['home'] = () => {
  ...
  // quick-tiles grid (Spotify "your top" shortcuts) — recent trips, clubs, crews
  const quick = [
    ['trip','bishop','Bishop Peak',I.trips],['club','field','Field Studies',I.comm],
    ['trip-hub','sykes','Sykes Hot Springs',I.tent],['messages','','Moonlight Crew',I.comm],
    ['trip','surf','Dawn Patrol Surf',I.trips],['club','surf','Surf Club',I.comm],
    ['trip','climb','Bouldering @ Pad',I.trips],['messages','','Beach Crew',I.comm],
  ];
  const byPill = (ids)=> ids.filter(id=> U.homePill==='all' ? true : (U.homePill==='activities'? TRIPS[id].kind==='Activity' : TRIPS[id].kind==='Adventure'));
  const shelf = (title, list, sub) => { const l=byPill(list); if(!l.length) return ''; return `
    <div class="section-h"><h3>${title}</h3><span class="more">See all</span></div>
    ${sub?`<div class="section-sub">${sub}</div>`:''}
    <div class="shelf">${l.map(id=>tripCard(TRIPS[id], true)).join('')}</div>`; };
  return `
  <div class="home-top">
    <button class="avatar-btn" data-nav="profile-me">${avatar('YU',34)}</button>
    <div class="pills">${pills.map(([id,l])=>`<button class="pill ${U.homePill===id?'on':''}" data-action="setHomePill" data-param="${id}">${l}</button>`).join('')}</div>
    <button class="roundbtn" data-nav="home-search" title="Search trips">${I.search}</button>
  ...
  <div class="scroll">
    ...
    ${shelf('Made for you', ['bishop','sykes','climb'], 'Picked from what you\'re into')}
...
/* ============================================================
   ...
   ============================================================ */
function streakBar(){
  const dots = [1,1,1,0,0]; // 3-week streak
  return `<div class="streak">
    <div class="avatar">${I.flame}</div>
    <div style="flex:1"><div class="streak__big">3-week streak</div>
      <div class="streak__dots">${dots.map(d=>`<i class="${d?'on':''}">${d?I.check:''}</i>`).join('')}</div></div>
    <div class="center"><div class="small muted">this season</div><div class="strong">keep it up</div></div>
  </div>`;
}
SCREENS['mytrips'] = () => {
...
SCREENS['communities'] = () => {
  ...
  const body = isYours
    ? `<div class="pad mt3">
        ...
        <div class="list">${yourCrews.map(c=>`<button class="row" style="width:100%;text-align:left" data-nav="messages"><div class="avatar" style="border-radius:12px;width:46px;height:46px">${I.comm}</div><div class="row__body"><div class="row__title">${c.name}</div><div class="row__sub">You + ${c.members-1} · ${c.sub}</div></div><span class="chev">${I.chev}</span></button>`).join('')}</div>
      </div>`
    : communitiesJoin();
  return `
  ${topbar('Communities')}
...
SCREENS['club'] = (p) => {
  ...
  const tabs = isMember ? [['landing','Landing'],['signups','Trip Signups'],['links','Useful Links'],['members','Members']]
  ...
  <div class="scroll" style="padding-top:0">
    ...
    <div class="toptabs sticky mt3" style="overflow-x:auto">${tabs.map(([id,l])=>`<div class="toptabs__i ${U.clubTab===id?'on':''}" style="white-space:nowrap" data-action="setClubTab" data-param="${id}">${l}${id==='members'&&!isMember?' '+I.lock:''}</div>`).join('')}</div>
    ${body}
...
const ACT_STEPS = [
  {q:'Name your trip', hint:'The most important part — keep it short and inviting.', field:()=>'<input class="field" placeholder="e.g. Bishop Peak Sunset Hike" />'},
  {q:'Describe it', hint:'What\'s the vibe? What should people expect? Room to write plenty.', field:()=>'<textarea class="field" style="min-height:150px" placeholder="Chill after-class hike, tacos after…"></textarea>'},
  {q:'General location', hint:'Type a common name — not an exact address.', field:()=>`<input class="field" placeholder="e.g. Baker Hall · Sequoia National Forest" />`},
  {q:'Who can see it?', hint:'', field:()=>`<div class="gap2">
      <button class="chip on" style="width:100%;justify-content:flex-start;padding:14px">Private — invite only</button>
      <button class="chip" style="width:100%;justify-content:flex-start;padding:14px">Group — list it under a club or crew you lead</button>
      <button class="chip" style="width:100%;justify-content:flex-start;padding:14px">Public — anyone can view & join</button>
      <div class="divlabel">which club?</div>
      <div style="display:flex;gap:8px;flex-wrap:wrap">${Object.values(CLUBS).map((c,i)=>`<button class="chip ${i===0?'on':''}">${c.name}</button>`).join('')}</div>
    </div>`},
  {q:'Budget', hint:'Keep it honest — even just "carpool for gas."', field:()=>`<input class="field" placeholder="e.g. $6 each — carpool for gas" />`},
  {q:'Add photos', hint:'1–6 shots so people know what they\'re in for.', field:()=>`<div class="tiles" style="padding:0;grid-template-columns:1fr 1fr 1fr">${[0,1,2,3,4,5].map(i=>`<button class="ph" style="height:96px;border-radius:12px" data-action="toast" data-param="Photo added">${I.plus}</button>`).join('')}</div>
      <label class="check mt5" style="border:none;padding:0" data-action="toast" data-param="Repeat weekly toggled"><span class="box">${''}</span><span class="small">Repeat weekly? (optional — e.g. a Thursday dip)</span></label>`},
];
...
SCREENS['plan-desktop'] = () => `
  <div class="dt">
    <div class="dt__side">
      ...
      <p class="small muted mt4">Adventures are overnight — you plan them on desktop where there's room for the budget maker and a full write-up.</p>
      <button class="btn btn--ghost btn--sm mt6" data-action="back">${I.back} Back</button>
    </div>
    <div class="dt__main">
      <div class="dt__bar" style="margin:-26px -34px 22px;padding-left:14px"><span class="dt__dot"></span><span class="dt__dot"></span><span class="dt__dot"></span><span class="small muted" style="margin-left:8px">Plan an Adventure</span></div>
      <h2 style="margin:0 0 4px">Mammoth Ski Weekend</h2>
      <p class="muted" style="margin:0 0 22px">Fill in the details, write it up, set the budget, publish to your club's Trip Signups.</p>
      <div class="form-grid">
        ${[['Trip name','Mammoth Ski Weekend'],['Dates','Mar 21–23'],['General location','Mammoth Lakes'],['Capacity','24 people'],['Who can see it','Ski & Snowboard (club)'],['Co-leads','+ add lead']].map(([l,v])=>`<div><div class="label">${l}</div><div class="field">${v}</div></div>`).join('')}
      </div>
      <div class="label mt6">The write-up (make it hefty — sell the trip)</div>
      <div class="field" style="min-height:90px;max-width:640px">Two nights in a cabin, lift tickets included, family-style dinners. We'll carpool up Friday after class…</div>
      ...
      <div class="label mt6">Photos</div>
      <div style="display:flex;gap:12px;max-width:520px">${[0,1,2,3].map(()=>`<div class="ph" style="width:110px;height:80px;border-radius:10px">${I.plus}</div>`).join('')}</div>
...
SCREENS['channel-chat'] = (p) => {
  ...
  const title = crew ? crew.name : (club? '# important' : '# important');
  return `
  ${pageHeader(title)}
  <div class="scroll pad">${bubbles([['MP','@channel Roster finalized for Sykes! Check your My Trips.',false],['JR','Let\'s gooo 🏕️',false],['','count me in',true]])}</div>
...
/* ============================================================
   ...
   ============================================================ */
SCREENS['profile-me'] = () => {
  const patches = [['Hot Springs',true],['Sunset Summit',true],['First Wave',true],['Powder Day',false],['Trip Lead',false]];
  return `
  ${pageHeader('Profile')}
  <div class="scroll">
    ...
    <div class="section-h"><h3 style="font-size:16px">Patches</h3><span class="more">one per trip</span></div>
    <div class="patchrow">${patches.map(([n,earned])=>`<div class="patchtile ${earned?'':'locked'}"><div class="disc">${earned?I.smile:I.patch}</div><div class="pn">${n}</div></div>`).join('')}</div>
...
function openPlanSheet(){
  openSheet(`
    ...
    <button class="optcard" data-action="startAdventure"><div class="avatar">${I.tent}</div><div><div class="strong" style="font-size:17px">Adventure</div><div class="small muted mt2">Overnight — you'll finish this one on desktop.</div></div></button>
...
Object.assign(ACTIONS, {
  ...
  addPhoto:()=>{ if(U.obPhotos<2){ U.obPhotos++; render(); } else toast('2 is plenty for now'); },
  usePrompt:(p)=>{ S.ob.bio = p+' '; render(); },
  ...
  startActivity:()=>{ closeSheet(); U.actIndex=0; navigate('plan-activity'); },
  startAdventure:()=>{ closeSheet(); navigate('plan-desktop'); },
