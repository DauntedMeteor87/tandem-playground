...
SCREENS['home'] = () => {
  ...
  const debriefCards = Object.values(DEBRIEFS).slice(0,5).map(d=>shelfCard({
    title: d.title,
    sub: `${avatar(pInit(d.poster),20)}<span class="tripcard__club">${PEOPLE[d.poster].name} · ${d.crew}</span>`,
    tag: d.when, meta: d.place, nav:'debriefs-list',
  }, true)).join('');
  ...
  <div class="scroll">
    ...
    ${section('Adventures for you', 'See more adventures', 'adventures-list', advCards)}
    ${section('Debriefs', 'See all debriefs', 'debriefs-list', debriefCards)}
...
SCREENS['debriefs-list'] = () => `
  ${pageHeader('Debriefs')}
  <div class="scroll"><div class="mt2">
    ${Object.values(DEBRIEFS).map(d=>`
    <div class="post">
      <div class="post__head">
        ${avatar(pInit(d.poster),34)}
        <div style="flex:1;min-width:0">
          <div class="row__title" style="font-size:14px">${PEOPLE[d.poster].name} <span class="muted" style="font-weight:500">· ${d.crew}</span></div>
          <div class="small muted">${d.place} · ${d.when}</div>
        </div>
      </div>
      ${phImg('', 360)}
      <div class="post__actions">${I.hearto}${I.cmt}${I.share}</div>
      <div class="post__cap"><b>${PEOPLE[d.poster].name.split(' ')[0]}</b> ${d.caption}</div>
    </div>`).join('')}
  </div><div style="height:16px"></div></div>`;
...
const ACT_STEPS = [
  ...
  {q:'Who can see it?', hint:'', field:()=>`<div class="gap2">
      ...
      <div style="display:flex;gap:8px;flex-wrap:wrap">${Object.values(CLUBS).map(c=>`<button class="chip ${U.actClub===c.id?'on':''}" data-action="setActClub" data-param="${c.id}">${c.name}</button>`).join('')}</div>
    </div>`},
...
Object.assign(ACTIONS, {
  ...
  actNext:()=>{ if(U.actIndex<ACT_STEPS.length-1){ U.actIndex++; render(); } else {
    S.members[U.actClub] = true;
    S.publishedTrip = { clubId:U.actClub, tripId:'bishop' };
    U.clubTab = 'messages';
    S.stack = [];
    navigate('club', U.actClub);
