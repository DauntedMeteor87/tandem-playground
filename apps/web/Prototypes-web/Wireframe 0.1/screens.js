/* ============================================================
   tandemclub web wireframe 0.1 — Plan an Adventure flow
   ============================================================ */
...
function topbar(label){
  return `<div class="topbar">
    <div class="topbar__brand">tandemclub <span class="muted">· web</span></div>
    <div class="topbar__meta">${label || 'Plan an Adventure — draft'}</div>
    <div class="topbar__actions">${label ? '' : `<button class="btn btn--ghost btn--sm" data-action="toast" data-param="Draft saved">Save &amp; exit</button>`}</div>
  </div>`;
}

function railHtml(){
  const i = stepIndex(S.step);
  return `<div class="wiz__rail">
    <div class="wiz__railhead">
      <div class="wiz__railtitle">${ADV.name}</div>
      <div class="wiz__railsub">Fill in the details, write it up, set the budget, then publish to your club's Trip Signups.</div>
    </div>
    ${STEPS.map((s,idx)=>{
...
function footerHtml(){
  ...
  return `<div class="wiz__foot">
    <div class="side">${isFirst ? '' : `<button class="btn btn--ghost" data-action="prevStep">${I.back} Back</button>`}</div>
    <div class="side">
      ${isLast
        ? `<button class="btn" data-action="openPublish">Publish to Trip Signups</button>`
        : `<button class="btn" data-action="nextStep">Continue</button>`}
    </div>
  </div>`;
}
...
function stepPreview(){
  ...
  <h1 class="wiz__title">Preview &amp; publish</h1>
  <p class="wiz__hint">Exactly what this looks like on the mobile app before it goes live.</p>
  <div class="preview-wrap">
    <div class="preview-copy">
      <div class="card">
        <div class="row"><span class="row__body"><span class="row__title">Ready to publish?</span></span></div>
...
function previewOverview(){
  ...
  <div class="dhero">${'<div class="ph"></div>'}<div class="dhero__grad"></div>
    <div class="dhero__meta"><div class="t">${ADV.name}</div><div class="s">${fmtDate(ADV.dateStart)}–${fmtDate(ADV.dateEnd)} · ${ADV.location}</div></div></div>
...
function renderConfirmation(){
  ...
  <div class="wiz">
    ...
    <div class="wiz__main"><div class="wiz__body">
      <div class="card confirm__card mt6">
        <div class="confirm__icon">${I.check}</div>
        <h2>Published to ${visLabel()}</h2>
        <p>"${ADV.name}" is now live in Trip Signups on the mobile app — members can find it and join.</p>
        <div style="display:flex;gap:10px;justify-content:center">
          <button class="btn btn--ghost" data-action="goHome">Go to Home</button>
...
/* ---------- Site-level sidebar (Home + Plan) — shown once you've published at least once ---------- */
function appSidebar(active){
  return `<div class="wiz__rail">
    <div class="wiz__railhead">
      <div class="wiz__railtitle">tandemclub</div>
      <div class="wiz__railsub">Web portal — trip planning</div>
    </div>
    <button class="railstep ${active==='home'?'current':''}" data-action="goHome">
      <span class="railstep__n">${I.home}</span>Home
    </button>
    <button class="railstep ${active==='plan'?'current':''}" data-action="startAnother">
      <span class="railstep__n">${I.plus}</span>Plan an Adventure
    </button>
  </div>`;
}

/* ---------- Home (landing) ---------- */
const CURRENT_LEADER = 'Maya'; // the mobile prototype's lead on Sykes Hot Springs (PEOPLE.maya) — same person, same trip
function renderHome(){
  const trips = S.publishedTrips;
  const upcoming = trips.filter(t => new Date(t.dateStart) >= new Date(new Date().setHours(0,0,0,0)));
  const totalSpots = trips.reduce((s,t)=>s+Number(t.capacity||0), 0);
  const clubCount = CLUBS.length + CREWS.length;

  return `
  ${topbar('Home')}
  <div class="wiz">
    ${appSidebar('home')}
    <div class="wiz__main"><div class="wiz__body" style="max-width:900px">
      <div class="home-hero">
        <div>
          <div class="wiz__eyebrow">Welcome back</div>
          <h1 class="wiz__title" style="margin-bottom:0">Hey, ${CURRENT_LEADER} 👋</h1>
        </div>
      </div>

      <div class="quick-tiles mt5">
        <button class="quicktile dark" data-action="toast" data-param="Club management is coming next">
          <div class="quicktile__eyebrow">Coming next</div>
          <div class="quicktile__title">${I.users} Manage your club</div>
          <div class="quicktile__sub">Rosters, roles &amp; club settings</div>
        </button>
        <button class="quicktile accent" data-action="startAnother">
          <div class="quicktile__eyebrow">New trip</div>
          <div class="quicktile__title">${I.plus} Plan an Adventure</div>
      ...
      <div class="stat-tiles mt5">
        ${[
          ['Adventures published', trips.length],
          ['Total spots offered', totalSpots],
          ['Upcoming', upcoming.length],
          ['Clubs & crews', clubCount],
          ...
          ${trips.map(t=>`
            <div class="triptile">
              ...
              <div class="triptile__body">
                <span class="tag">Live · ${t.visLabel}</span>
                <div class="triptile__name">${t.name}</div>
                <div class="row__sub">${fmtDate(t.dateStart)}–${fmtDate(t.dateEnd)} · ${t.location} · ${t.capacity} spots</div>
              </div>
            </div>`).join('')}
        </div>
      ` : `
        <div class="banner">${I.info} Nothing published yet — plan your first adventure and it'll show up here.</div>
      `}
    </div></div>
  </div>`;
}
...
Object.assign(ACTIONS, {
  ...
  doPublish: ()=>{
    ...
    S.publishedTrips.unshift({ name: ADV.name, dateStart: ADV.dateStart, dateEnd: ADV.dateEnd, location: ADV.location, capacity: ADV.capacity, visLabel: visLabel() });
    render();
  },
  goHome: ()=>{ S.appView = 'home'; render(); },
  startAnother: ()=>{
    ADV = freshAdventure(); S.published = false; S.appView = 'wizard';
    S.step = 'basics'; S.visited = new Set(['basics']); S.previewTab='overview';
    render();
  },
});
